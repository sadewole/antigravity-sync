...
class SyncService {
    constructor(authService, githubService, outputChannel) {
    ...
    async collectFiles(dir, base = '') {
        ...
        for (const file of list) {
            ...
            if (stat && stat.isDirectory()) {
                ...
                if (file === 'node_modules' || file === '.git' || file === 'workspaceStorage')
                    continue;
        ...
        for (const file of files) {
            ...
            if (file.path.startsWith('vscode/')) {
                if (file.path === 'vscode/extensions.json') {
                    ...
                    const remoteExtensions = JSON.parse(file.content);
